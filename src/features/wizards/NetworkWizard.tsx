 ...
 * 3-step visual interface for creating Netplan network configurations
 * - Step 1: Interface selection
 * - Step 2: DHCP or Static IP configuration
 * - Step 3: Review and apply
 */
export function NetworkWizard({ isOpen, onClose, onApply }: NetworkWizardProps) {
  const [step, setStep] = useState(1);
  const [interfaceName, setInterfaceName] = useState('eth0');
  const [configType, setConfigType] = useState<ConfigType>('dhcp');
  ...
  const commonInterfaces: CommonInterface[] = [
