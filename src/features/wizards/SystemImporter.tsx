...
interface ImportData {
  ...
  usersOutput: string;
  timezoneOutput: string;
  localeOutput: string;
}

interface SystemImporterWizardProps {
  isOpen: boolean;
  onClose: () => void;
...
/**
 * System Importer Utility
 * Parses command outputs from existing Ubuntu systems
 * and generates autoinstall configurations
 */
export const SystemImporter = {
  /**
   * Parse hostname from hostnamectl output
   */
  parseHostname(output: string): string | null {
   ...
   */
  parseNetworkInterfaces(output: string): NetworkInterface[] {
   ...
   */
  parsePackages(output: string): string[] {
   ...
   */
  parseUsers(output: string): UserInfo[] {
   ...
   */
  parseTimezone(output: string): string | null {
   ...
   */
  parseLocale(output: string): string | null {
   ...
   */
  generateConfig(data: ParsedSystemData): any {
    const config: any = {
      version: 1
    };
 ...
 * 5-step wizard for importing configuration from existing system
 * - Step 1: System information (hostname, timezone, locale)
 * - Step 2: Network configuration
 * - Step 3: Installed packages
 * - Step 4: User accounts
 * - Step 5: Review and apply
 */
export function SystemImporterWizard({ isOpen, onClose, onApply }: SystemImporterWizardProps) {
  ...
  const [parsedData, setParsedData] = useState<ParsedSystemData>({});
  ...
  const handleNext = () => {
    if (step === 1) {
      // Parse system info
      const parsed: ParsedSystemData = {};
      if (importData.hostnameOutput) {
        const hostname = SystemImporter.parseHostname(importData.hostnameOutput);
        if (hostname) parsed.hostname = hostname;
      }
      if (importData.timezoneOutput) {
        const timezone = SystemImporter.parseTimezone(importData.timezoneOutput);
        if (timezone) parsed.timezone = timezone;
      }
      if (importData.localeOutput) {
        const locale = SystemImporter.parseLocale(importData.localeOutput);
