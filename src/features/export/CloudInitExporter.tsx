...
/**
 * CloudInitExporter utility object
 * Converts autoinstall configurations to cloud-init format
 */
export const CloudInitExporter = {
  /**
   * Convert autoinstall config to cloud-init format
   */
  convertToCloudInit(autoinstallConfig: AutoinstallConfig): CloudInitConfig {
    ...
    if (autoinstallConfig.identity) {
      const users: CloudInitUser[] = [];

      if (autoinstallConfig.identity.username) {
        const user: CloudInitUser = {
    ...
    if (autoinstallConfig.keyboard) {
      cloudConfig.keyboard = {
        layout: autoinstallConfig.keyboard.layout || 'us'
      };
   ...
   */
  generateYAML(autoinstallConfig: AutoinstallConfig): string {
   ...
   */
  getSummary(autoinstallConfig: AutoinstallConfig): ConversionSummary {
 ...
 * Exports autoinstall configuration to cloud-init format
 */
interface CloudInitExportModalProps {
  isOpen: boolean;
  onClose: () => void;
  autoinstallYaml: string;
}

export function CloudInitExportModal({ isOpen, onClose, autoinstallYaml }: CloudInitExportModalProps) {
  ...
  useEffect(() => {
    if (isOpen && autoinstallYaml) {
      try {
        ...
        setError(`Error converting to cloud-init: ${errorMessage}`);
        setCloudInitYaml('');
        setSummary(null);
      }
    }
