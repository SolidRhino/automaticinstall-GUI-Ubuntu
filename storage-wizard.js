...
// Visual interface for creating storage configurations

const StorageWizard = ({ isOpen, onClose, onApply }) => {
    const [step, setStep] = React.useState(1);
    const [scheme, setScheme] = React.useState('simple');
    const [diskPath, setDiskPath] = React.useState('/dev/sda');
    ...
    const [lvmConfig, setLvmConfig] = React.useState({
        vgName: 'ubuntu-vg',
        volumes: [
            { name: 'root', size: '30G', mount: '/' },
            { name: 'swap', size: '4G', mount: 'swap' },
            { name: 'home', size: '100%FREE', mount: '/home' }
        ]
    });
    ...
    const generateStorageConfig = () => {
        ...
        if (scheme === 'custom') {
            // Generate custom partition config
            const config = {
                config: []
            };
                    ...
                    <div className="space-y-6">
                        <div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                {Object.entries(storagePresets).map(([key, preset]) => (
                                    <div
                                        key={key}
...
if (typeof window !== 'undefined') {
    window.StorageWizard = StorageWizard;
}
